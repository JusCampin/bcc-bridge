-------------------------------------------------------------------
-- bcc-bridge VORP  –  Server-side test suite
--
-- HOW TO USE
--   In the server console:
--       vorp_test_all <playerId>      – runs every test for that player
--       vorp_test_economy <playerId>  – money / inventory only
--       vorp_test_character <playerId> – identity + job + gang
...
-- Run a test: fn() must return (ok, value)
...
local function testJobs(src)
    ...
    run('GetAllJobs (table)', function()
        ...
        return ('keys: %d'):format(#all)
    end)
end

local function testGangs(src)
    header('GANGS (via metadata)')
...
local function testMetadata(src)
    ...
    run('SetMetadata (test_key)', function()
        local ok = Bridge:SetMetadata(src, 'vorp_test_key', { value = 42 })
        assert(ok ~= false, 'SetMetadata returned false')
        return ok
    end)

    run('GetMetadata (test_key)', function()
        local v = Bridge:GetMetadata(src, 'vorp_test_key')
        assert(type(v) == 'table' and v.value == 42, 'Value mismatch: ' .. tostring(v))
        return json.encode(v)
    end)

    -- Cleanup
    Bridge:SetMetadata(src, 'vorp_test_key', nil)
end
...
local function testEconomy(src)
    ...
    run('AddMoney (cash +500)', function()
        local ok = Bridge:AddMoney(src, 'cash', 500, 'bcc-bridge vorp test')
    ...
    run('RemoveMoney (cash -500)', function()
        local ok = Bridge:RemoveMoney(src, 'cash', 500, 'bcc-bridge vorp test')
        assert(ok ~= false, 'RemoveMoney returned false')
        return ok
    end)

    run('RemoveMoney insufficient funds (cash -99999999)', function()
        local ok = Bridge:RemoveMoney(src, 'cash', 99999999, 'bcc-bridge vorp test')
    ...
    run('AddMoney (gold +2)', function()
        local ok = Bridge:AddMoney(src, 'gold', 2, 'bcc-bridge vorp test')
        assert(ok ~= false, 'AddMoney gold returned false')
        return ok
    end)

    run('RemoveMoney (gold -2)', function()
        local ok = Bridge:RemoveMoney(src, 'gold', 2, 'bcc-bridge vorp test')
...
local function testNotify(src)
    ...
    run('Notify (primary)',  function() Bridge:Notify(src, '[bcc-bridge vorp test] primary',  'primary',  3000); return 'sent' end)
    run('Notify (success)',  function() Bridge:Notify(src, '[bcc-bridge vorp test] success',  'success',  3000); return 'sent' end)
    run('Notify (error)',    function() Bridge:Notify(src, '[bcc-bridge vorp test] error',    'error',    3000); return 'sent' end)
    run('Notify (warning)',  function() Bridge:Notify(src, '[bcc-bridge vorp test] warning',  'warning',  3000); return 'sent' end)
end

local function testCallback()
    header('CALLBACKS')
    run('RegisterCallback (vorp_test_cb)', function()
        Bridge:RegisterCallback('vorp_test_cb', function(src, args, ret)
...
local function runAll(src)
    print('\n^3╔══════════════════════════════════════╗^0')
    print('^3║  bcc-bridge VORP TEST SUITE (server) ║^0')
    print('^3╚══════════════════════════════════════╝^0')
    testFramework()
    testPlayer(src)
    testCharacter(src)
...
RegisterCommand('vorp_test_all', function(_src, args)
    local target = tonumber(args[1])
    if not target then print('[bcc-bridge vorp test] Usage: vorp_test_all <playerId>'); return end
    runAll(target)
end, true)

RegisterCommand('vorp_test_economy', function(_src, args)
    local target = tonumber(args[1])
    if not target then print('[bcc-bridge vorp test] Usage: vorp_test_economy <playerId>'); return end
    testEconomy(target)
    testInventory(target)
end, true)

RegisterCommand('vorp_test_character', function(_src, args)
    local target = tonumber(args[1])
    if not target then print('[bcc-bridge vorp test] Usage: vorp_test_character <playerId>'); return end
    testPlayer(target)
...
RegisterCommand('vorp_test_meta', function(_src, args)
    local target = tonumber(args[1])
    if not target then print('[bcc-bridge vorp test] Usage: vorp_test_meta <playerId>'); return end
    testMetadata(target)
    testLicenses(target)
    testSkills(target)
end, true)
