-------------------------------------------------------------------
-- bcc-bridge RSG  –  Client-side test suite
--
-- HOW TO USE  (in-game chat commands, requires ace permission or open perms)
--   /rsg_test_client_all      – runs every instant + async test
--   /rsg_test_client_instant  – instant / cached exports only
--   /rsg_test_client_async    – Server* async exports only
--
-- Results are printed to the client's F8 console.
-- Player data populates on the 'rsg-core:onPlayerLoaded' event; run
-- these tests after fully spawning in to avoid 'not loaded yet' results.
-------------------------------------------------------------------
...
local function testInstant()
    ...
    run('GetPlayerData', function()
        ...
        if not d then return 'nil (spawn pending — expected before rsg-core:onPlayerLoaded)' end
    ...
    run('Notify (primary)', function()
        Bridge:Notify('[bcc-bridge rsg client test] primary', 'primary', 3000)
        return 'sent'
    end)

    run('Notify (success)', function()
        Bridge:Notify('[bcc-bridge rsg client test] success', 'success', 3000)
        return 'sent'
    end)

    run('Notify (error)', function()
        Bridge:Notify('[bcc-bridge rsg client test] error', 'error', 3000)
        return 'sent'
    end)

    run('Notify (warning)', function()
        Bridge:Notify('[bcc-bridge rsg client test] warning', 'warning', 3000)
        return 'sent'
    end)

    -- TriggerCallback test against the 'rsg_test_cb' registered in server/test.lua
    run('TriggerCallback (rsg_test_cb)', function()
        Bridge:TriggerCallback('rsg_test_cb', function(result)
            if result then
                pass('TriggerCallback → response', result)
            else
                fail('TriggerCallback → response', 'nil result')
            end
        end, { msg = 'hello from rsg client' })
        return 'triggered (response async)'
    end)

    -- TriggerCallbackAwait test (blocking)
    run('TriggerCallbackAwait (rsg_test_cb)', function()
        local result = Bridge:TriggerCallbackAwait('rsg_test_cb', { msg = 'await test' })
        assert(result ~= nil, 'Got nil from await')
        return result
    end)
end
...
local function testAsync()
    ...
    runAsync('ServerAddMoney (cash +100)', function(done)
        Bridge:ServerAddMoney('cash', 100, 'bcc-bridge rsg async test', function(result)
            done(result, function() end)
        end)
    end)

    runAsync('ServerRemoveMoney (cash -100)', function(done)
        Bridge:ServerRemoveMoney('cash', 100, 'bcc-bridge rsg async test', function(result)
            done(result, function() end)
        end)
    end)
    ...
    Bridge:ServerNotify('[bcc-bridge rsg async test] ServerNotify sent', 'success', 3000)
...
RegisterCommand('rsg_test_client_all', function()
    Citizen.CreateThread(function()
        print('\n^3╔════════════════════════════════════════╗^0')
        print('^3║  bcc-bridge RSG TEST SUITE (client)    ║^0')
        print('^3╚════════════════════════════════════════╝^0')
        header('INSTANT EXPORTS')
        testInstant()
        header('ASYNC EXPORTS')
        testAsync()
        print('\n^2All RSG client tests complete.^0\n')
    end)
end, false)

RegisterCommand('rsg_test_client_instant', function()
    Citizen.CreateThread(function()
        header('INSTANT EXPORTS')
        testInstant()
        print('^2Done.^0')
...
RegisterCommand('rsg_test_client_async', function()
    Citizen.CreateThread(function()
        header('ASYNC EXPORTS')
        testAsync()
        print('^2Done.^0')
